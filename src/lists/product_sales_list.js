...
import { getProductSales, getProductStock, getProducts, updateProduct } from "../controllers/product_controller";
...
export const ProductSalesList = ({ refresh, uuid }) => {
  ...
  const filteredProducts = sales.filter((item) =>
...
const [showOffcanvans, setshowOffcanvans] = useState(false);
  ...
  return (
    <div className=" p-3" style={{ backgroundColor: "white" }}>
      ...
      <Heading fontSize={"1.5vw"} text={"sales"} />
          ...
          <tbody>
            {filteredProducts.slice(indexOfFirstProduct, indexOfLastProduct).map((item, index) => (
              <tr key={index}>
                ...
                <td>{item.price * item.amount + "TZS"}</td>
