...
const Paragraph = ({text,color,fontWeight,fontSize,maxLines,className, onClick,style}) => {
    return (
        ...
        <div onClick={onClick} className={className+ " d-block d-md-none"} style={{ color: color??blackColor,fontWeight:fontWeight??400,fontSize:"3vw",maxLines:maxLines??3,...style }} dangerouslySetInnerHTML={{ __html:text }}></div>
