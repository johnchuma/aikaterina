...
const Heading = ({text,color,fontWeight,fontSize,style,className}) => {
    ...
    <div>
        ...
        <div className={className+" d-block d-md-none"} style={{lineHeight:1.1, color: color??blackColor,fontWeight:fontWeight??600,fontSize:"6.5vw",...style}} dangerouslySetInnerHTML={{ __html:text }} ></div>
    </div>
    )
}
